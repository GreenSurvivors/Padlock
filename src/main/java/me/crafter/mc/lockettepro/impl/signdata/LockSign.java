...
public class LockSign {
    private final static NamespacedKey storedMembersUUIDKey = new NamespacedKey(LockettePro.getPlugin(), "MemberUUIDs");
    private final static NamespacedKey storedOwnersUUIDKey = new NamespacedKey(LockettePro.getPlugin(), "OwnerUUIDs");
    ...
    public static void updateNamesFromUUIDs(@NotNull Sign sign) {
        ...
        if (owners != null && !owners.isEmpty()) {
            try {
                ...
                LockettePro.getPlugin().getLogger().log(Level.WARNING, "broken UUID \"" + owners.get(0) + "\" (invalid) in Lock-Sign at " + sign.getLocation(), e);
            }
        } else {
            LockettePro.getPlugin().getLogger().log(Level.WARNING, "Lock sign without Owners at " + sign.getLocation());
        }
        ...
        if (members != null && !members.isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                try {
                    ...
                    LockettePro.getPlugin().getLogger().log(Level.WARNING, "broken UUID \"" + members.get(i) + "\" (invalid) in Lock-Sign at " + sign.getLocation(), e);
                }
            } //for loop
        } //empty members
